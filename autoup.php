...
define('QUICK_LOGIN', 'http://test.site/pagelogin.php?qlogin=90e763c53c9da922304a5aef982f7c5b533f0770efbe259161c87d0a889bcf3d320bc88839ba8a8e93a29dcb19896631');

define('TMDB_API', '2a2c0a98175ae7646878c600d8f744f7');
...
function make_upload($file_full, $ext, $new_dir)
{
    ...
    case preg_match('/s\d+e\d+|s\d+|hdtv|sdtv|pdtv|tvrip/i', $file) : $cat = 5;
        break;
    case preg_match('/xvid|brrip|bluray|dvdrip|hdrip/i', $file) : $cat = 10;
        break;
    case preg_match('/x86|x64|win64|lnx64|macosx/i', $file) : $cat = 1;
        break;
    case preg_match('/wii|wiiu|xbox|xbox360|ps3|ps4/i', $file) : $cat = 2;
        break;
    case preg_match('/dvdr/i', $file) : $cat = 3;
        break;
    case preg_match('/mp3|flac|lossless|cd|compilation|album|albums|vinyl/i', $file) : $cat = 4;
        break;
    case preg_match('/xxx/i', $file) : $cat = 6;
        break;
    case preg_match('/psp/i', $file) : $cat = 7;
        break;
    case preg_match('/ps2/i', $file) : $cat = 8;
        break;
    case preg_match('/anime/i', $file) : $cat = 9;
        break;
    case preg_match('/720p|1080p/i', $file) : $cat = 11;
        break;
    case preg_match('/pc/i', $file) : $cat = 12;
        break;
    default : $cat = 9;
    }
    ...
    if (TMDB_API != '') {
        if (($cat === 10) || ($cat === 11)) {
            ...
            switch(true)
            {
            case preg_match('/^\d+.[a-z.]+.\d+/i', $file_name) : preg_match('/\d+.[a-z.]+.\d+/i', $file_name, $matching);
                break;
            case preg_match('/^\d+.\d+/', $file_name) : preg_match('/\d+.\d+/', $file_name, $matching);
                break;
        ...
        if ($cat === 5) {
            ...
            switch(true) 
            {
            case preg_match('/^\d+.[a-z.]+.s\d+e\d+./i', $file_name) : preg_match('/\d+.[a-z.]+.s\d+e\d+./i', $file_name, $matching);
                break;
            case preg_match('/^[a-z.]+.\d+.s\d+e\d+./i', $file_name) : preg_match('/[a-z.]+.\d+.s\d+e\d+./i', $file_name, $matching);
                break;
            case preg_match('/^\d+.[a-z.]+.s\d+./i', $file_name) : preg_match('/\d+.[a-z.]+.s\d+./i', $file_name, $matching);
                break;
            case preg_match('/^[a-z.]+.\d+.s\d+./i', $file_name) : preg_match('/[a-z.]+.\d+.s\d+./i', $file_name, $matching);
                break;
            case preg_match('/^[a-z.]+.s\d+./i', $file_name) : preg_match('/[a-z.]+.s\d+./i', $file_name, $matching);
                break;
...
function scan_folder()
{
    ...
    if (!is_dir($dir_done)) {
        $ok = mkdir($dir_done);
        if (!$ok) {
            die('Cannot create destination folder!');
        }
    }
